...
public class StreamFilter {
    public List<Employee> filterUsingLambda(List<Employee> employees) {
        return employees.stream().filter(e -> e.getAge() > 50).toList();
    }

    public List<Employee> filteringUsingMethodReference(List<Employee> employees) {
        return employees.stream().filter(Employee::isVaccinated).toList();
    }

    public List<Employee> filteringUsingMultipleConditions(List<Employee> employees) {
        return employees.stream().filter(e -> e.getAge() > 50 && e.isVaccinated()).toList();
    }
    ...
    public static void main(String[] a) {
        ...
        streamFilter
                .handlingExceptionsInFilterThrowingFunction(employees)
